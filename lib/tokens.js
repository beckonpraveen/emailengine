...
const { redis, mysqlConnection } = require('./db');
...
module.exports = {
    async provision(opts) {
        console.log("CSK provisioning access tokens")
        opts = opts || {};
        ...
        if (account) {
            ...
            console.log("Going to add account to mysql");
            const uniqueRandomID = uuid.v4()
            let sql = `INSERT INTO iat (id, token, account) VALUES ?`
            var values = [[uniqueRandomID,hashedToken, account]]
            mysqlConnection.query(sql, [values], function (err, result) {
                if (err) throw err;
                console.log("Number of records inserted: " + result.affectedRows);
              });
            try{
                await prisma.iat.create({
                    data:{
                        account:account,
                        token: hashedToken
                    }
                ...
                connsole.log("Prisma iat token created");
            }
            ...
            const uniqueRandomID = uuid.v4()
            let sql = `INSERT INTO iat (id, token, account) VALUES ?`
            var values = [[uniqueRandomID,hashedToken, null]]
            mysqlConnection.query(sql, [values], function (err, result) {
                if (err) throw err;
                console.log("Number of records inserted: " + result.affectedRows);
              });
