...
// const {  createAccountData } = require("emailengine-addon")
const { prisma } = require("../prisma");
// const {createAccount, updateAccount, deleteAccount} = require("../prisma/utils")
const { tryCatch } = require('bullmq');
// const sayHello = require("emappplugin")
const { createAccount, updateAccount, deleteAccount, createIfNotExist } = require("./orm-account")


// const alien1 = new eerdbms(this.redis, prisma)
...
class Account {
    ...
    async loadAccountData(account, requireValid) {
        console.log("CSK: Load account Data latest called")
        ...
        if (!result || !result.account) {
            let dbaccountData = await createIfNotExist(result, this.getAccountKey());

            // console.log("CSK info /rmal dbaccountData ", dbaccountData)

            if (dbaccountData) {
                console.log("Before serializing", dbaccountData)

                let nser = this.serializeAccountData(dbaccountData)
                console.log("CSK dbaccountData.smtp ", dbaccountData.imapServerInfo, " -- ", JSON.stringify(dbaccountData.imapServerInfo))
                nser.imapServerInfo = (dbaccountData.imapServerInfo) ? JSON.stringify(dbaccountData.imapServerInfo) : dbaccountData.imapServerInfo
                nser.oauth2 = (dbaccountData.oauth2) ? JSON.stringify(dbaccountData.oauth2) : dbaccountData.oauth2
                nser.lastErrorState = (dbaccountData.lastErrorState) ? JSON.stringify(dbaccountData.lastErrorState) : dbaccountData.lastErrorState
                nser.smtp = (dbaccountData.smtp) ? JSON.stringify(dbaccountData.smtp) : dbaccountData.smtp
                nser.imap = (dbaccountData.imap) ? JSON.stringify(dbaccountData.imap) : dbaccountData.imap


                console.log("Loading data to redis ", this.serializeAccountData(dbaccountData));
                console.log("Loading nser ", nser);
                // this.create(dbaccountData);
                let pipeline = this.redis
                    .multi()
                    .hgetall(this.getAccountKey())
                    .hmset(this.getAccountKey(), nser)
                ...
                console.log("CSK before exec ", result)

                let resultExe = await pipeline.exec();
                result = await this.redis.hgetall(this.getAccountKey());

                // console.log("CSK after exec ",resultExe)
            }
        }
        ...
        console.log("CSK dbaccountData ", "  --- ", accountData);
    ...
    async update(accountData) {
        ...
        this.logger.info({ msg: 'CSK update account old', accounts: oldAccountData });
        ...
        await updateAccount(this.account, this.serializeAccountData(accountData));
        ...
        console.log('CSK update account 2', this.account);
    ...
    async create(accountData) {
        console.trace()
        console.log("CSK accountData on init ", accountData)
        ...
        console.log("CSK accountData mid", this.redis
            .multi().get(this.getAccountKey()))

        let serializedAccountData = this.serializeAccountData(accountData)

        await createAccount(serializedAccountData);


        // console.log("CSK accountData on init ", this.serializeAccountData(accountData))
    ...
    async delete() {
        ...
        await deleteAccount(this.account)
    ...
    async createMailbox(path) {
        ...
        console.log("praveen ", this.account, " path ", path);
        await this.loadAccountData(this.account, true);
    ...
    async getMessage(message, options) {
        ...
        if (options.documentStore && (await settings.get('documentStoreEnabled'))) {
            ...
            if (messageData.text._generatedHtml) {
