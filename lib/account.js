...
class Account {
    ...
    async update(accountData) {
        ...
        try {
            const dbaccount = await prisma.iad.findFirst({ where: { account: this.account } })
            if (dbaccount) {
                ...
                const account = await prisma.iad.update({
                    ...
                    data: {
                        account: (serializedAccountData?.account) ? serializedAccountData?.account : "",
                        sync: new Date().toISOString(),
                        email: (serializedAccountData?.email) ? serializedAccountData?.email : "",
                        name: (serializedAccountData?.name) ? serializedAccountData?.name : "",
                        statecountconnected: '0',
                        syncFrom: (serializedAccountData?.syncFrom) ? serializedAccountData?.syncFrom : 'null',
                        tz: (serializedAccountData?.tz) ? serializedAccountData?.tz : "",
                        notifyFrom: (serializedAccountData?.notifyFrom) ? serializedAccountData?.notifyFrom : new Date().toISOString(),
                        imap: accountData?.imap,
                        imapServerInfo: (serializedAccountData?.imapServerInfo) ? serializedAccountData?.imapServerInfo : {},
                        state: "init",
                        smtp: (serializedAccountData?.smtp) ? serializedAccountData?.smtp : "",
                        lastErrorState: (serializedAccountData?.lastErrorState) ? serializedAccountData?.lastErrorState : "",
                        statecountsyncing: (serializedAccountData?.statecountsyncing) ? serializedAccountData?.statecountsyncing : "",
                        oauth2: (serializedAccountData?.oauth2) ? serializedAccountData?.oauth2 : ""
                    }
                })
            }
        } catch (error) {
    ...
    async create(accountData) {
        ...
        console.log("CSK serializedAccountData ",serializedAccountData);
    ...
    async delete() {
        ...
        try {
            ...
            console.log("CSK account deleted on ", deletedbaccount)
        } catch (error) {
