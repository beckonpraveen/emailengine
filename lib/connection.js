...
class Connection {
    ...
    async setStateVal() {
        ...
        let localData  = {}
        localData[`statecount${this.state}`] = incrVal+""
    ...
    async start() {
        ...
        try {
            ...
            if (featureFlags.enabled('gmail api')) {
                if (imapConfig.type === 'api') {
                    // TODO: handle API accounts
                    logger.info({ msg: 'API user connection', imapConfig });
                    return;
                }
            }
    ...
    async renameMailbox(path, newPath) {
        this.checkIMAPConnection();
        try {
            let result = await this.imapClient.mailboxRename(path, newPath);
            if (result) {
                result.renamed = !!result.newPath;

                try {
                    await this.imapClient.mailboxSubscribe(result.newPath);
                } catch (err) {
    ...
    async queueMessageEntry(data, meta, licenseInfo) {
                ...
                if (attachmentsToDownload && attachmentsToDownload.length) {
                    ...
                    for (let attachment of attachmentsToDownload) {
                        ...
                        if (attachment.content) {
                            ...
                            // fetch from IMAP
                            content = await this.getAttachmentContent(attachment.id, { chunkSize: Math.max(DOWNLOAD_CHUNK_SIZE, 2 * 1024 * 1024) });
                        }
                        if (!content) {
                            // skip missing?
                            continue;
        ...
        for (let attachment of data.attachments || []) {
            if (attachment.reference && !attachment.content) {
                ...
                let content = await this.getAttachmentContent(attachment.reference, { chunkSize: Math.max(DOWNLOAD_CHUNK_SIZE, 2 * 1024 * 1024) });
                if (!content) {
